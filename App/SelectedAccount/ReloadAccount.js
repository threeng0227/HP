import React, { Component } from "react";
import { Text, StyleSheet, View, Image,TouchableOpacity } from "react-native";
import {
  Container,
  Header,
  Left,
  Body,
  Footer,
  Button,
  Icon,
  Title,
  FooterTab,
  Content,
  CheckBox,
  Right
} from "native-base";
import Ionicons from 'react-native-vector-icons/Ionicons';
import Ic from 'react-native-vector-icons/FontAwesome';
import Icons from 'react-native-vector-icons/FontAwesome5';

export default class ReloadAccount extends Component {
  constructor() {
    super();
    this.state = {
      check: true,
      checkic: true
    };
  }
  onePressed() {
    this.setState({ check: true, check1: false, check2: false });
  }
  twoPressed() {
    this.setState({ check: false, check1: true, check2: false });
  }
  threePressed() {
    this.setState({ check: false, check1: false, check2: true });
  }
  iconPressed() {
    alert("asdsa");
  }
  render() {
    return (
      <Container>
        <Header style={{ backgroundColor: "white" }}>
                <Left>
            <View style={{ justifyContent: "center"}}>
                <Button transparent style={{justifyContent: "center" }} onPress={()=>{this.props.navigation.goBack()}}><Icons size={20}  name="chevron-left"></Icons></Button>
            </View>
            </Left>
           <Body  style={{justifyContent: "center" ,alignItems:"center",flex:4}}>
               <Text style={{ fontSize: 20,  fontWeight: "bold" }}>Select Account</Text>
           </Body>
           <Right style={{justifyContent: "center" ,alignItems:"center",flexDirection:'row'}}>
           <Text><Ic name="bell" size={20}></Ic></Text>

           <TouchableOpacity style={{paddingLeft:'25%'}} onPress={()=>{this.props.navigation.openDrawer()}}>
           <Ionicons name="md-menu" size={25}></Ionicons>
                   </TouchableOpacity>
           </Right>
      </Header>
        <Content padder style={{ backgroundColor: "white" }}>
          <View
            style={{
              height:'19%',
              flexDirection: "column",
              justifyContent: "space-around",
              paddingHorizontal: "2%",
              borderWidth: 0.5,
              width: "94%",
              borderColor: "gray",
              marginLeft: "4%",
              borderRadius: 6,
              flex :1
            }}
          >
            <View
              style={{
                flexDirection: "row",
                justifyContent: "space-between",
                alignItems: "flex-start",
            
              }}
            >
              <Left>
                <Text style={{ marginLeft: "5%", fontSize: 17 }}>Amount:</Text>
                <Text
                  style={{
                    marginVertical: "4%",
                    marginLeft: "5%",
                    fontSize: 17
                  }}
                >
                  Tip:
                </Text>
                <View style={{ flexDirection: "row" }}>
                  <CheckBox checked={true} style={{ marginRight: "9%" }} />
                  <Text>Use gift card(Value : $ 0)</Text>
                </View>
              </Left>
              <Right>
                <Text style={{ fontSize: 17, fontWeight: "bold" }}>
                  $ 150.00
                </Text>
                <Text
                  style={{
                    marginVertical: "4%",
                    fontSize: 17,
                    fontWeight: "bold"
                  }}
                >
                  $ 27 .00
                </Text>
                <Text style={{ fontSize: 17, fontWeight: "bold" }}>
                  -$ 0.00
                </Text>
              </Right>
            </View>
            <View
              style={{
                width: "100%",
                borderWidth: 1,
                borderColor: "#EEEEEE",
                marginVertical: "3%"
              }}
            />
            <View
              style={{ flexDirection: "row", justifyContent: "space-around" }}
            >
              <Left>
                <Text style={{fontSize: 17, fontWeight: "bold"}}>Total:</Text>
              </Left>
              <Right>
                <Text style={{ fontSize: 17, fontWeight: "bold",color:'#0764b0' }}>
                  $ 177.00
                </Text>
              </Right>
            </View>
          </View>

          <View style={{flexDirection: "column",flex :1.5,paddingVertical:'2%'}}>
            <View
              style={{
                marginLeft: "4%",
                marginRight: "4%",
                borderRadius: 4,
                borderWidth: 1,
                borderColor: "#EEEEEE",
                paddingTop: "5%",
                padding: "3%",
                height:'100%'
              }}
            >
              <Text
                style={{ fontSize: 18, color: "black", marginBottom: "3%" }}
              >
                Select the way to pay
              </Text>
              <View
                style={{
                  height: 47,
                  flexDirection: "row",
                  alignItems: "center",
                  flex: 1,
                  justifyContent: "center",
                  marginBottom: "6%"
                }}
              >
                <View style={{ flex: 1, marginLeft: "-4%" }}>
                  <CheckBox
                    style={site.check}
                    checked={this.state.check}
                    onPress={() => this.onePressed()}
                  />
                </View>

                <View style={{ flex: 1, marginLeft: "-2%" }}>
                  {/*                   <Image
                    style={{ height: 50, width: 50 }}
                    source={require("../Appointments/images/Group2.png")}
                  /> */}
                </View>
                <View style={{ flex: 2, marginRight: "-7%" }}>
                  <Text style={{ color: "black" }}>Harmony Account</Text>
                  <Text style={{ color: "black" }}>JERRY NGUYEN</Text>
                </View>
                <View
                  style={{
                    flex: 1,
                    marginRight: "-8%",
                    marginTop: "-7%",
                    color: "lightgreen"
                  }}
                >
                  <Text
                    style={{
                      fontSize: 13,
                      fontWeight: "bold",
                      color: "lightgreen"
                    }}
                  >
                    $ 914.00
                  </Text>
                </View>
              </View>
              <View
                style={{
                  height: 47,
                  flexDirection: "row",
                  alignItems: "center",
                  flex: 1,
                  justifyContent: "center",
                  marginBottom: "6%",
                  marginBottom: "6%"
                }}
              >
                <View style={{ flex: 1, marginLeft: "-4%" }}>
                  <CheckBox
                    style={site.check}
                    checked={this.state.check1}
                    onPress={() => this.twoPressed()}
                  />
                </View>

                <View style={{ flex: 1, marginLeft: "-8%" }}>
                  {/* <Image
                    fontSize={30}
                    source={require("../Appointments/images/thevisaa.png")}
                  /> */}
                </View>
                <View style={{ flex: 2, marginLeft: "-3%" }}>
                  <Text style={{ color: "black" }}>
                    **** **** **** **** 1234
                  </Text>
                  <Text style={{ color: "black" }}>JERRY NGUYEN</Text>
                </View>
              </View>
              <View
                style={{
                  height: 47,
                  flexDirection: "row",
                  alignItems: "center",
                  flex: 1,
                  justifyContent: "center",
                  marginBottom: "6%"
                }}
              >
                <View style={{ flex: 1, marginLeft: "-4%" }}>
                  <CheckBox
                    style={site.check}
                    checked={this.state.check2}
                    onPress={() => this.threePressed()}
                  />
                </View>

                <View style={{ flex: 1, marginLeft: "-5%" }}>
                  {/*                   <Image
                    style={{ height: 50, width: 50 }}
                    source={require("../Appointments/images/nganhang2.png")}
                  /> */}
                </View>
                <View style={{ flex: 2, marginLeft: "-5%" }}>
                  <Text style={{ color: "black" }}>Bank America</Text>
                  <Text style={{ color: "black" }}>
                    **** **** **** **** 5565
                  </Text>
                </View>
              </View>
              <View
                style={{
                  paddingTop: "3%",
                  height: 47,
                  flexDirection: "row",
                  alignItems: "center",
                  flex: 1,
                  justifyContent: "center",
                  marginBottom: "6%"
                }}
              >
                <View style={{ flex: 1 }}>
                  <Icon
                    style={site.ic}
                    name="md-add-circle"
                    check={this.state.checkic}
                    onPress={() => this.iconPressed()}
                  />
                </View>
                <View style={{ flex: 3 }}>
                  <Text style={{ color: "black" }}>
                    Add a bank account or card
                  </Text>
                </View>
              </View>
            </View>

          </View>
          <View style={{alignItems:'center',flex:2,marginBottom:'15%'}} >
            <Text style={{color:'red',marginBottom:'2%'}} >Your accounts is not enough to pay .Please reload!</Text>
                <Button style={site.btn} onPress={()=>{this.props.navigation.navigate('ReloadAccountStep2')}}>
                  <Text style={site.btn1}> Reload </Text>
                </Button>
            </View>
        </Content>
        
      </Container>
    );
  }
}
var site = StyleSheet.create({
  ic: {
    paddingTop: "8%",
    color: "#00B2EE",
    marginLeft: "-2%",
    fontSize: 38,
    flex: 1
  },
  check: {
    borderRadius: 30,
    height: 32,
    width: 32,
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 0,
    borderColor: "#EEEEEE"
  },
  btn: {
    backgroundColor: "#2680eb",
    width: "40%",
    alignSelf:'center',
    borderRadius: 6,
    justifyContent:'center'
 
  },
  btn1: {
    color: "white",
    fontSize: 16,
    fontWeight:'700'
    
  },
  view: {
    marginTop: "35%",
    flex: 1,
    justifyContent: "center",
    alignItems: "center",
    flexDirection: "row"
  }
});
